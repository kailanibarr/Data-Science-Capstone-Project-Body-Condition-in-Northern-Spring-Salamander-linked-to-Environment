...
palette(c(cols[1:6]))
ggplot(data=salamander, aes(x=SVL, y=TrunkWidth, col="black")) + ggtitle("Body Measurement Variation by Stage") + xlab("Snout Vent Length") + ylab("Trunk Width") + geom_point() +scale_color_manual(values = c("Larvae" = cols[2],"Adult"=cols[6])) + geom_abline(intercept = -0.243396,slope = 0.167269, colour = cols[2]) + geom_abline(intercept = 0.194793,slope = 0.149023, colour = cols[6]) + theme(text=element_text(family = "Helvetica Neue Light"),plot.title = element_text(face = "bold"))
```
...
plot(Stage~RockSize, data = training.full, xlab="Rock Size in cm", ylab ="Stage", cex.main=1.2, main="Log Odds Probability of Larval Stage Given Rock Size")
...
lines(l3, col=cols[6])
lines(l4, col=cols[8])
legend("bottomleft",c("Wetted Edge & Riffle", "Thalweg & Riffle", "Wetted Edge & Pool", "Thalweg & Pool"),fill = c(cols[2], cols[4], cols[6], cols[8]),border = "black", inset = c(0.05, 0.08))
```
...
par(family = "Helvetica Neue Light")
plot(training.full$SVL~training.full$RockSize, col = training.full$Stage, xlab = "Rock Size in cm", ylab = "SVL in cm", main = "Salamander Stage Distribution of SVL by Rock Size", cex.main=1.2)
...
m1<-data.frame( Model = "Model 1",
            R2 = R2(pred.1, testing.full$SVL),
            RMSE = RMSE(pred.1, testing.full$SVL),
            MAE = MAE(pred.1, testing.full$SVL))
m2<-data.frame( Model = "Model 2",
            R2 = R2(pred.2, testing.full$SVL),
            RMSE = RMSE(pred.2, testing.full$SVL),
            MAE = MAE(pred.2, testing.full$SVL))
# m3<-data.frame( Model = "Model 3",
#             R2 = R2(pred.3, testing.full$SVL),
#             RMSE = RMSE(pred.3, testing.full$SVL),
#             MAE = MAE(pred.3, testing.full$SVL))
m4<-data.frame( Model = "Model 4",
            R2 = R2(pred.4, testing.full$SVL),
            RMSE = RMSE(pred.4, testing.full$SVL),
            MAE = MAE(pred.4, testing.full$SVL))
rbind(m1,m2,m4)
```
